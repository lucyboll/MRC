    ...
    with tf.Session(config=config) as sess:
        ...
        while current_step < hps.max_run_steps:
            ...
            train_data, batch_c_word, batch_q_word, batch_c_chars, batch_q_chars, batch_target, batch_c_lens, \
                batch_q_lens = get_batch(train_data)
            print('-', end='')
            step_loss, summary, global_step = model.run_trian_step(sess, batch_c_word, batch_q_word, batch_c_chars,
                                                                   batch_q_chars, batch_c_lens, batch_q_lens,
                                                                   batch_target)
            ...
            if current_step % hps.steps_per_checkpoint == 0:
                ...
                dev_data, batch_c_word, batch_q_word, batch_c_chars, batch_q_chars, batch_target, batch_c_lens, \
                    batch_q_lens = get_batch(dev_data)
...
def main(_):
    ...
    char_emb = np.reshape(char_emb, (hps.char_vocab_size, hps.emb_size))
    print('char_emb ok!!!')
    hps.mode = FLAGS.mode
    if FLAGS.mode == 'train':
