...
def load_glove(dir_, name, vocab_size):
	glove = np.zeros((vocab_size, hps.emb_size), dtype=np.float32)
	with codecs.open(dir_, "rb", "utf-8") as f:
		...
		while line:
			...
			if len(vector) != hps.emb_size + 1:
				...
				continue
			name_ = vector[0]
			vector = vector[1:hps.emb_size+1]
			if vector:
				try:
			...
			i += 1
	print("\n")
	glove_map = np.memmap(hps.data_dir + name + ".np", dtype='float32', mode='write', shape=(vocab_size, hps.emb_size))
	glove_map[:] = glove
	del glove_map
				...
				with codecs.open(question_chars_dir, 'rb', 'utf-8') as q_c_f:
					with codecs.open(target_dir, 'r', 'utf-8') as t_f:
						...
						while c_word and q_word and c_chars and q_chars and target:
							...
							for c_c, q_c in zip(c_chars, q_chars):
								splited_c_chars.append(c_c.split())
								splited_q_chars.append(q_c.split())
							c_lens = len(c_word)
							q_lens = len(q_word)
							padded_c_word = c_word + [0] * (hps.max_c_len - c_lens)
							padded_q_word = q_word + [0] * (hps.max_q_len - q_lens)
							padded_c_chars = splited_c_chars + [[]] * (hps.max_c_len - len(splited_c_chars))
							padded_q_chars = splited_q_chars + [[]] * (hps.max_q_len - len(splited_q_chars))
							_padded_c_chars, _padded_q_chars = [], []
							assert len(padded_c_chars) == hps.max_c_len
							assert len(padded_q_chars) == hps.max_q_len
							for c_c in padded_c_chars:
								padded_c_c = c_c + [0] * (hps.max_char_len - len(c_c))
								if len(padded_c_c) > hps.max_char_len:
									padded_c_c = padded_c_c[:hps.max_char_len]
								_padded_c_chars.append(padded_c_c)
							for q_c in padded_q_chars:
								padded_q_c = q_c + [0] * (hps.max_char_len - len(q_c))
								if len(padded_q_c) > hps.max_char_len:
									padded_q_c = padded_q_c[:hps.max_char_len]
							...
							data.append((padded_c_word, padded_q_word, _padded_c_chars, _padded_q_chars, target, c_lens, q_lens))
...
def get_batch(data):
	...
	while count < hps.batch_size:
		c_word, q_word, c_chars, q_chars, target, c_lens, q_lens = data.get()
		data.put((c_word, q_word, c_chars, q_chars, target, c_lens, q_lens))
	...
	batch_c_chars = np.reshape(np.asarray(batch_c_chars, dtype=np.int32),
							   newshape=(hps.batch_size * hps.max_c_len, hps.max_char_len))
	batch_q_chars = np.reshape(np.asarray(batch_q_chars, dtype=np.int32),
							   newshape=(hps.batch_size * hps.max_q_len, hps.max_char_len))
	return (data, np.asarray(batch_c_word, dtype=np.int32), np.asarray(batch_q_word, dtype=np.int32), batch_c_chars,
			batch_q_chars, _batch_target, np.array(batch_c_lens, dtype=np.int32), np.array(batch_q_lens, dtype=np.int32))
...
if __name__ == '__main__':
	...
	train_data, batch_c_word, batch_q_word, batch_c_chars, batch_q_chars, batch_target, batch_c_lens, batch_q_lens = \
